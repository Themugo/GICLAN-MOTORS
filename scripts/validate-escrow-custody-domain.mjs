...
const checks = [
  ...
  ['private seller only at payment boundary', fs.readFileSync('backend/controllers/paymentController.js','utf8').includes('validatePrivateSellerEscrow') && fs.readFileSync('backend/services/escrowConfiguration.service.js','utf8').includes('individual_seller')],
  ...
  ['dealer listings cannot enable escrow', fs.readFileSync('backend/controllers/carController.js','utf8').includes('Vehicle purchase escrow is a private-seller custody rail') && fs.readFileSync('backend/controllers/carController.js','utf8').includes('req.body.escrowEnabled = false')],
