...
const checks = [
  ...
  ['payment initiated after local record creation', /authoritative payment row BEFORE contacting the provider/.test(fs.readFileSync(path.join(root, 'backend/services/paymentService.js'), 'utf8'))],
  ...
  ['callback amount mismatch is finalized', /Amount mismatch: expected/.test(fs.readFileSync(path.join(root, 'backend/services/paymentCallback.service.js'), 'utf8')) && /markWebhookProcessed\(webhookEventId\)/.test(fs.readFileSync(path.join(root, 'backend/services/paymentCallback.service.js'), 'utf8'))],
