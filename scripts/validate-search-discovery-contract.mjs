...
const filterBar = read('src/components/MarketplaceFilterBar.tsx');
const showroom = read('src/pages/Showroom.jsx');
const browse = read('src/pages/BrowsePage.jsx');
const mobileBrowse = read('src/pages/mobile/MobileBrowsePage.jsx');
...
assert.match(search, /getSearchFacets/);
for (const field of ['brand','model','location','body','fuel','transmission','color','condition','dealerType','priceMin','priceMax','yearMin','yearMax','mileageMax','verifiedOnly','inspectedOnly']) assert.match(filterBar, new RegExp(field));
assert.match(showroom, /searchMarketplace/);
assert.match(browse, /Navigate.*showroom/);
assert.match(mobileBrowse, /Navigate.*showroom/);
assert.doesNotMatch(navbar, /id="county-selector-top"/);
assert.doesNotMatch(navbar, /Region: <strong/);
